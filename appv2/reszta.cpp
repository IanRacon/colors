		    ...
		    double *zeta, double *psi, double *cos_psi, double *vxn1, double *vyn1, double *dt_cfl)
{
	...
	for (int j = 0; j < ny; j++)
	{
	    ...
	    vyn1[l] = 0.;
	    if ((j != 0) && (j != (ny - 1)))
		vxn1[l] = (psi[l + 1] - psi[l - 1]) / 2. / dx;
	    if ((i != 0) && (i != (nx - 1)))
		vyn1[l] = -(psi[l + ny] - psi[l - ny]) / 2. / dx;
	}
    }
		 ...
		 double *a, int *jka, int *iwa, double *b, int *jkb, int *iwb)
{
    ...
    int nstart, minimum, m, i, j, l;
    double alfa1, alfa2, alfa3, alfa4, alfa5;
    double beta1, beta2, beta3, beta4, beta5;

    //.....wypelniamy A_n+1 - wierszami...............

    m = -1; //licznik dziala od zera
	...
	for (j = 0; j < ny; j++)
	{
	    ...
	    alfa5 = minimum;
	    if (i > 0)
		alfa1 = -dt / 4. / dx * vxn1[l];
	    if (j > 0)
		alfa2 = -dt / 4. / dx * vyn1[l];
	    alfa3 = 1.;
	    if (j < (ny - 1))
		alfa4 = dt / 4. / dx * vyn1[l];
	    if (i < (nx - 1))
		alfa5 = dt / 4. / dx * vxn1[l];
