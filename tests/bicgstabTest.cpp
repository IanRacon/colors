...
{
TEST(procedureConjugateGradient, bicgstab_shouldReturnRightVector_for3x3)
{
    int size = 3;
    ...
    std::vector<double> result = bicgstab(csrMatrix, b, x0);
    for (int i = 0; i < size; ++i)
    {
        ASSERT_NEAR(expected.at(i), result.at(i), 0.0001);
    }
}
TEST(procedureConjugateGradient, bicgstab_shouldReturnRightVector_for6x6)
{
    int size = 6;
...
TEST(procedureConjugateGradient, bicgstab_shouldConverge_for1000x1000)
{
    int size = 1000;
    std::vector<double> b;
    b.reserve(size);
